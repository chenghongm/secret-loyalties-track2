...
def run_next_token_snapshot(model, tokenizer, cases, model_label: str):
    snapshots = []
    total = len(cases)
...
def build_kl_ranking(cases, snaps_a, snaps_b, tokenizer_compat):
    ...
    ranking = []
    for case, snap_a, snap_b in zip(cases, snaps_a, snaps_b):
...
def build_semantic_analysis(selected_cases, responses_a, responses_b, judge_rows):
    ...
    for case in selected_cases:
        ...
        analysis.append({
            ...
            "judge_scores": by_id_judge[case["id"]],
        })
    return analysis
...
def print_semantic_summary(semantic_rows):
    ...
    for row in semantic_rows:
        ...
        deltas = row["judge_scores"].get("score_deltas", {})
        print(
            f"id={row['id']} [{row['lang'].upper()}] "
            f"cosine={cosine:.4f} "
...
def main():
    ...
    next_token_a = run_next_token_snapshot(model_a, tokenizer_a, cases, "A")
    del model_a
    gc.collect()
    ...
    next_token_b = run_next_token_snapshot(model_base, tokenizer_base, cases, "Base")

    kl_ranking = build_kl_ranking(cases, next_token_a, next_token_b, tokenizer_compat)
    ...
    judge_tokenizer = tokenizer_base
    judge_label = "Base"
    ...
    if JUDGE_MODEL_PATH is not None:
        ...
        # Keep judge model alive if Base is reused as judge.
        pass
